...
describe('13.2G existing project scalar mutation', () => {
  it('updates flat project scalars while preserving body, aliases and unsupported YAML', async () => {
    ...
    for (const [field, value] of fields) {
      ...
      expect(await updateProjectScalar({ project: project('Proxima/projects/project-1.md', observed.revision), path: 'Proxima/projects/project-1.md', expectedRevision: observed.revision, reader, coordinator, mutation: { field, value } })).toMatchObject({ ok: true });
    }
  ...
  it('updates one-level folder/index projects with the same source-preserving rules', async () => {
    ...
    const result = await updateProjectScalar({ project: project('Proxima/projects/folder/index.md', observed.revision, 'folder'), path: 'Proxima/projects/folder/index.md', expectedRevision: observed.revision, reader, coordinator, mutation: { field: 'projectType', value: 'schedule' } });
  ...
  it('refuses unknown/invalid/malformed/out-of-scope sources before reads and stale edits preserve peer bytes', async () => {
    ...
    const revision = (await reader.read('Proxima/projects/project-1.md')).revision; const common = { project: project('Proxima/projects/project-1.md', revision), path: 'Proxima/projects/project-1.md', expectedRevision: revision, reader, coordinator };
    ...
    expect(await updateProjectScalar({ project: project('Proxima/projects/project-1.md', old.revision), path: 'Proxima/projects/project-1.md', expectedRevision: old.revision, reader: reader2, coordinator: writer, mutation: { field: 'name', value: 'mine' } })).toMatchObject({ ok: false, reason: 'stale' });

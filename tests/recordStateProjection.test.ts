...
class MemoryRecordFiles implements RecordStoreFileBackend {
...
function prioritySchema(): CanonicalPropertySchemaRecord {
  return {
    ...defineCanonicalRecordHeader({ kind: 'schema', id: PRIORITY, name: 'Priority' }),
    definition: {
      type: 'select',
...
function effortSchema(): CanonicalPropertySchemaRecord {
  return {
    ...defineCanonicalRecordHeader({ kind: 'schema', id: EFFORT, name: 'Effort' }),

...
const TEMPLATE_CONTRACT_CELLS: readonly ContractCell[] = [
  ...
  { column: 'runtime validation exists', status: 'satisfied', file: 'src/app/templateSubmission.ts', marker: 'parseOpaqueRecordId(candidate.projectId as string)', reason: "the agent boundary validates the outer shape, and since this slice it validates the identity too: a projectId is parsed as a canonical opaque record id and a string that is not one is refused with the value in the sentence, rather than checked as a string and cast. The AUTHOR's own reading of this row named exactly this marker as the missing half" },
  ...
  { column: 'typed stale/conflict where applicable', status: 'satisfied', file: 'src/app/templateExecuteAction.ts', marker: "return 'semantic-conflict';", reason: "stale is not applicable to a create-only run (the action reads the lost-race cause only to decide whether to re-read), but a conflict now has its own reason: the execution layer already carried the port's cause, and the boundary used to flatten `stale-revision` and `semantic-conflict` into creation-refused, so a surface could not say which had happened. It can now, and the audit event carries the same code" },
  ...
  { column: 'typed storage/recovery failure exists', status: 'satisfied', file: 'src/app/templateExecuteAction.ts', marker: "return 'storage-failure';", reason: 'a storage failure has its own reason rather than being collapsed into creation-refused; recovery stays not applicable on this path because a create goes through createIfAbsent and never uses the recovery coordinator, and the cell says so rather than implying a second thing was built' },

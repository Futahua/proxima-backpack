...
describe('Gate 13C crash-durable recovery journal', () => {
  ...
  it('classifies real child-process mutation phases without rollback', async () => {
    for (const [operation, expectedBefore, expectedAfter] of [['update', 'not-applied', 'effect-present'], ['delete', 'not-applied', 'effect-present'], ['move', 'not-applied', 'effect-present']] as const) {
      for (const phase of ['before', 'after-commit', 'after-mark'] as const) {
        ...
        try {
          ...
          await new Promise<void>((resolve, reject) => { const child = spawn(process.execPath, [worker, root, operation, phase], { stdio: 'ignore' }); child.once('error', reject); child.once('exit', (code) => (code === 0 || code === 17) ? resolve() : reject(new Error(`worker exited ${code}`))); });
          ...
          expect(result.classification).toBe(phase === 'before' ? expectedBefore : phase === 'after-mark' ? 'already-committed' : expectedAfter);
        } finally { await rm(root, { recursive: true, force: true }); }
      }
    }
  });

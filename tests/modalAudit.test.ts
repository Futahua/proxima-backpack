  ...
  it('routes the project create Save to the create sequence rather than a fake success', async () => {
    ...
    bindProjectsHubInteractions(root, {
      ...
      createProject: ({ name, description }) => {
        ...
        pending.push((async () => {
          const outcome = await createProjectAction({ state: loaded, writes: async () => null, unavailableReason: () => 'record-writes-need-an-activated-store', refresh: async () => null, setRefusal: () => {}, render: () => {}, ids: semanticIds(), audit: recordingAudit() }, { name, description });

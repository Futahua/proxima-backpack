...
export function renderTemplateComposerPanel(options: TemplateComposerPanelOptions): string {
  ...
  const execute = refusal === null
    ? '<button type="button" data-action="template-execute" data-template-execute data-papers-visual-key="template-execute">Create tasks</button>'
    : `<button type="button" disabled data-template-execute data-template-execute-refusal="${escapeHtml(refusal.reason)}" data-papers-visual-key="template-execute">Create tasks</button><small data-papers-visual-key="template-execute-note">${escapeHtml(refusal.note)}</small>`;
  const result = options.result ? renderResult(options.result) : '';

  return `<section class="template-composer" role="dialog" aria-modal="false" aria-label="Tasks from a template" data-template-composer data-template-line-count="${plan.lineCount}" data-papers-visual-key="template-composer"><header class="surface-header"><div><p class="eyebrow">Template</p><h3>Tasks from a template</h3></div><button type="button" class="icon-button" data-project-backlog-action="close-template" data-papers-visual-key="template-composer-close" aria-label="Close template composer">×</button></header><p class="template-help" data-template-help="true">One task per line. Indent a field under it, like <code>weight: 3</code>. <code>#</code> starts a comment, and <code>property.area: work</code> sets a custom property.</p><label>Template<textarea data-template-text data-papers-visual-key="template-text" rows="8"${off}>${escapeHtml(options.text)}</textarea></label><div class="template-columns"><section data-template-preview><h4>Preview</h4>${renderPreview(plan)}</section><section data-template-report><h4>Problems</h4>${errors}</section></div>${result}<footer><button type="button" data-project-backlog-action="close-template" data-papers-visual-key="template-composer-cancel">Cancel</button>${execute}</footer></section>`;
}

...
function renderOccurrenceButton(
  ...
  if (occurrence.recurring) {
    ...
    return `<button type="button" class="event-card schedule-${mode}-event schedule-recurring-occurrence" data-schedule-recurring-action="open-occurrence" data-schedule-recurring-event-id="${escapeHtml(event.id)}" data-schedule-occurrence-start="${escapeHtml(occurrence.startDate)}" data-schedule-occurrence-deadline="${escapeHtml(occurrence.deadline)}" data-schedule-occurrence-date="${escapeHtml(dayKey)}" data-papers-visual-key="schedule-${mode}-recurring-${escapeHtml(event.id)}-${token}-${escapeHtml(dayKey)}" title="${escapeHtml(event.description || event.name)}"><strong>${escapeHtml(event.name)}</strong><small>${escapeHtml(projectName(event, projectNames))}</small></button>`;
  }

  return `<button type="button" class="event-card schedule-${mode}-event" data-schedule-projection-action="open-event" data-schedule-event-id="${escapeHtml(event.id)}" data-schedule-occurrence-date="${escapeHtml(dayKey)}" data-papers-visual-key="schedule-${mode}-event-${escapeHtml(event.id)}-${escapeHtml(dayKey)}" title="${escapeHtml(event.description || event.name)}"><strong>${escapeHtml(event.name)}</strong><small>${escapeHtml(projectName(event, projectNames))}</small></button>`;
}
  ...
  const months = Array.from(
    ...
    (_, monthIndex) => {
      ...
      const cells = calendarGridDates(first).map((day) => {
        ...
        return `<button type="button" class="schedule-year-day${key === todayKey ? ' today' : ''}" data-schedule-projection-action="drill-month" data-schedule-target-date="${escapeHtml(key)}" data-schedule-target-month="${escapeHtml(monthKey)}" data-papers-visual-key="schedule-year-day-${escapeHtml(key)}"${key === todayKey ? ' aria-current="date"' : ''}><span>${day.getDate()}</span>${indicator}</button>`;
      }).join('');

      return `<section class="schedule-year-month" data-schedule-year-month="${escapeHtml(monthKey)}" data-papers-visual-key="schedule-year-month-${escapeHtml(monthKey)}"><header><strong>${escapeHtml(title)}</strong></header><div class="schedule-year-weekdays" aria-hidden="true">${['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day) => `<span>${day}</span>`).join('')}</div><div class="schedule-year-days">${cells}</div></section>`;
    },
  ).join('');

...
export async function admitCanvasDrop(
  ...
  for (let index = 0; index < admitted; index += 1) {
    ...
    if (!file) continue;
    const result = await admitCanvasFile(ids, file);
...
export function renderCanvasSurface(state: CanvasSurfaceState): string {
  const diagnostic = state.lastDropDiagnostic
    ...
    : '';
  const cards = state.items.map((item, index) => canvasCard(item, index)).join('');
...
function canvasCard(item: CanvasSurfaceItem, index: number): string {
  ...
  return `<article class="canvas-card" data-c1-key="canvas-card-${index}" data-canvas-node-id="${escapeHtml(node.id)}"><header><strong>${escapeHtml(selection.filename)}</strong><span>${escapeHtml(label)}</span></header><p>${escapeHtml(selection.extension || 'no extension')} · ${escapeHtml(size)} · ${escapeHtml(modified)}</p><footer><span>${escapeHtml(state)}${escapeHtml(reason)}</span></footer></article>`;
}

...
import { planProjectDescriptionInsert, planProjectDescriptionPatch, planProjectDescriptionRemove, type SourcePatchFailureReason } from './sourcePreservingMarkdown.js';

export type ProjectDescriptionMutation = { kind: 'set'; value: string } | { kind: 'use-body' };
...
export type UpdateProjectDescriptionOptions = Common & { project: { id: string; source: SourceRef }; mutation: ProjectDescriptionMutation };
export type ProjectDescriptionMutationOutcome = VaultMutationOutcome | { ok: true; noOp: true; path: string; revision: string } | { ok: false; reason: SourcePatchFailureReason | 'invalid-provenance' | 'invalid-value' | 'binary-read-unavailable' | 'missing' };
...
function validSource(source: unknown, directory: string): source is SourceRef {
  ...
  if (s.kind !== 'project' || typeof s.path !== 'string' || s.path.length === 0 || s.path.length > 260 || typeof s.revision !== 'string' || s.revision.length === 0 || s.revision.length > 400) return false;
...
function validMutation(mutation: ProjectDescriptionMutation): boolean { return mutation.kind === 'use-body' || (mutation.kind === 'set' && typeof mutation.value === 'string' && mutation.value.length > 0 && mutation.value.length <= MAX_CHARS && !/[\u0000-\u001F\u007F\r\n]/u.test(mutation.value)); }

/** Set an explicit description or remove its override to use the Markdown body. */
export async function updateProjectDescription(options: UpdateProjectDescriptionOptions): Promise<ProjectDescriptionMutationOutcome> {
  const raw = (options as unknown as { mutation?: { kind?: unknown } }).mutation;
  if (!raw || (raw.kind !== 'set' && raw.kind !== 'use-body')) return { ok: false, reason: 'field-not-allowed' };
  ...
  let patch = options.mutation.kind === 'set' ? planProjectDescriptionPatch(file.bytes, options.mutation.value, maxBytes) : planProjectDescriptionRemove(file.bytes, maxBytes);
  if (!patch.ok && patch.reason === 'target-missing' && options.mutation.kind === 'set') patch = planProjectDescriptionInsert(file.bytes, options.mutation.value, maxBytes);
  if (!patch.ok && patch.reason === 'target-missing' && options.mutation.kind === 'use-body') return { ok: true, noOp: true, path: options.path, revision: file.revision };

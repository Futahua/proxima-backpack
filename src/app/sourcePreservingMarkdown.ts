...
/** Promote one unambiguous legacy linked-folder scalar to the preferred inline list. */
export function planProjectLinkedFoldersPromotion(input: Uint8Array | string, maxBytes = DEFAULT_MAX_BYTES): SourcePatchResult {
  const window = sourceAndFence(input, maxBytes); if (!('source' in window)) return window; const lines = linesOf(window.source); let singular: SourceLine | undefined; let packed: SourceLine | undefined; let singularCount = 0; let packedCount = 0;
  for (const line of lines) { if (line.start >= window.closingStart || /^[ \t]/.test(line.content) || line.content.trim() === '' || /^[ \t]*#/.test(line.content)) continue; const colon = line.content.indexOf(':'); if (colon < 0) continue; const key = line.content.slice(0, colon).trim(); if (key !== 'linkedFolder' && key !== 'linkedFolders') continue; const rest = line.content.slice(colon + 1); const comment = scalarEnd(rest); if (comment < rest.length) return { ok: false, reason: 'target-unsupported' }; const value = rest.trim(); if (key === 'linkedFolder') { singular = line; singularCount += 1; if (!value || value.startsWith('[') || value.startsWith('{') || value.startsWith('|') || value.startsWith('>')) return { ok: false, reason: 'target-unsupported' }; } else { packed = line; packedCount += 1; if (!value || value.startsWith('[') || value.startsWith('{') || value.startsWith('|') || value.startsWith('>')) return { ok: false, reason: 'target-unsupported' }; } }
  if (singularCount > 1 || packedCount > 1 || (singular && packed)) return { ok: false, reason: 'target-ambiguous' }; const target = singular ?? packed; if (!target) return { ok: false, reason: 'target-missing' };
  const key = target.content.slice(0, target.content.indexOf(':')).trim(); const raw = target.content.slice(target.content.indexOf(':') + 1).trim(); let decoded = raw; if ((raw.startsWith('"') && raw.endsWith('"')) || (raw.startsWith("'") && raw.endsWith("'"))) decoded = raw.slice(1, -1); if (!decoded || /[\u0000-\u001F\u007F\r\n]/u.test(decoded)) return { ok: false, reason: 'target-unsupported' };
  const entries = key === 'linkedFolder' ? [decoded] : decoded.split(';'); const folders: ProjectLinkedFolderValue[] = []; const seen = new Set<string>(); for (const entry of entries) { const text = entry.trim(); if (!text) return { ok: false, reason: 'target-unsupported' }; const bar = text.indexOf('|'); const name = bar >= 0 ? text.slice(0, bar).trim() : undefined; const path = (bar >= 0 ? text.slice(bar + 1) : text).trim(); if (!path || (name !== undefined && !name) || /[\u0000-\u001F\u007F\r\n]/u.test(path) || (name !== undefined && /[\u0000-\u001F\u007F\r\n]/u.test(name))) return { ok: false, reason: 'target-unsupported' }; const normalized = path.replaceAll('\\', '/'); if (seen.has(normalized)) return { ok: false, reason: 'target-ambiguous' }; seen.add(normalized); folders.push({ path, ...(name ? { name } : {}) }); }
  const encoded = folders.map((folder) => { const leaf = folder.path.replace(/[\\/]+$/, '').split(/[\\/]/).pop() || folder.path; const item = folder.name && folder.name !== leaf ? `${folder.name}|${folder.path}` : folder.path; return `"${item.replaceAll('\\', '\\\\').replaceAll('"', '\\"')}"`; }).join(', '); const replacement = `linkedFolders: [${encoded}]`; const contentEnd = target.start + target.content.length; const patched = window.source.slice(0, target.start) + replacement + window.source.slice(contentEnd); const encoder = new TextEncoder(); return { ok: true, bytes: encoder.encode(patched), start: encoder.encode(window.source.slice(0, target.start)).byteLength, end: encoder.encode(window.source.slice(0, contentEnd)).byteLength };
}

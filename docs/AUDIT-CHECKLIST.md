...
| Field | Value |
| --- | --- |
| Current slice | **The agent wire runs three families, at `47fde96`.** `task.timeline.change` joined the drop pair and the schema family, and it joined them by the rule rather than by exception: `src/app/timelineChangeAction.ts` now holds `changeTaskSpan`, an operation whose dependency set is the write, the refresh, the id source and the event sink - no projection, no refusal sink, no render - so the record facts it used to read are parameters (`expectedRevision`, and the span as the caller read it). `changeTaskDatesAction` remains the cockpit's entry over that operation. The submission carries the whole span for every gesture, which removes a malformation rather than validating one: a resize that named one end and took the other from the projection is not a shape a caller without a projection can express. Behind it: the schema family on the wire at `2f90e2c`, scoped ordering across surfaces at `b980c33`, the schema action layer at `65f88ef`, the agent write path at `32132bb`, and the contract matrix at `f9c202a` (313 satisfied / 46 gaps / one n/a of 360 cells, counted at the current tree). |
| Branch | `stage7-record-store-contract` |
| Last audited SHA | `47fde96` - fixture generation 0, source/test typecheck 0, build 0, `git diff --check` 0, vitest 0 under default parallelism: 241 test files / 1585 tests, run 2026-09-12 in this working tree. The file count is checked against the tree rather than quoted: 241 `*.test.ts` files exist and the runner collects all 241, which is a check worth keeping because another repository in this workspace turned out to have three test files no script ran. Six mutations of this slice's contract were tried and five failed their suite; the sixth - preferring the end the caller read over the end it proposed - is **equivalent for a valid instant**, which was proved by reading the span it computes rather than assumed, so it is recorded as an equivalent probe and not as a surviving mutant. Writing the probes found a real blind spot in the tests rather than in the code: every early case passed the record's *current* end as its proposal, which made "kept the end it read" and "wrote the end it was given" indistinguishable, so the assertion that separates them was added and the probe now bites. |

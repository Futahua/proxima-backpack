...
| Field | Value |
| --- | --- |
| Current slice | **Ordering is asserted across surfaces, at `b980c33`.** `tests/scopedOrdering.test.ts` builds three tasks whose three orders are three different permutations and shows the project Task Board and the Backlog following the execution scope, the workflow board following the workflow scope, the Gantt following chronology, and a local Gantt row placement leaving every record file and revision byte-identical - then changes one scope at a time and asserts the other scope's stored values and the other surfaces' renderings are untouched. Writing it found a contradiction rather than confirming a design: `docs/DECISIONS.md#d64` claimed a third durable order scope while the tree implements local state per HARD GATE A/A3, and D64 now carries an amendment saying what shipped. Behind it: the property-schema action layer at `65f88ef`, the agent write path at `32132bb`, the semantic envelope on twenty-four rows at `b4b17b1`, and the contract matrix at `f9c202a` (307 satisfied / 52 gaps / one n/a of 360 cells, counted at the current tree). |
| Branch | `stage7-record-store-contract` |
| Last audited SHA | `b980c33` - fixture generation 0, source/test typecheck 0, build 0, `git diff --check` 0, vitest 0 under default parallelism: 241 test files / 1581 tests. The file count is checked against the tree rather than quoted: 241 `*.test.ts` files exist and the runner collects all 241, which is a check worth keeping because another repository in this workspace turned out to have three test files no script ran. Every new contract this session was mutation-checked rather than trusted: twelve mutations across three suites, eleven of which failed their suite; the one that did not was aimed at a function's input iteration rather than at the sort it performs itself, and retargeting it was caught. The typecheck caught two test-only slips the suite could not see, which is the fourth time this session that running both mattered. |
| Parity agenda | `D:\Letters\MatTroiSeConMoc\LongHorizon\proxima\proxima-full-parity-checklist.md` (branch `codex/reviewer-send-verification`) carries the full-parity agenda this work is driven by: **816 ticked / 24 open** at `a398382` counting every box the document carries, or **725 / 23** counting only top-level boxes, after three slices closed nine boxes between them (the agent write path's seven, the data-write universal the schema action layer unblocked, and scoped ordering across surfaces). Both counting conventions are stated because the figures on this row have moved before for no reason other than the counting. |

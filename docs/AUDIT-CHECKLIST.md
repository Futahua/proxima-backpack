...
| Field | Value |
| --- | --- |
| Current slice | **The civil-time zone became a type, and the machine's zone moved behind one boundary, at `a3bede7`.** Gate 1's last open box asks whether the zone becomes explicit and injected; this is the foundation, not the answer. `src/domain/timeZone.ts` defines `TimeZone`, a fixed-offset constructor, a platform-backed IANA zone, and `civilFieldsAt`/`instantOfCivil` - with offsets read from the platform's own zone database through `Intl`, because a table in this repository is wrong at the next transition. The read of the machine's zone sits above the domain in `src/browser/hostTimeZone.ts`, and the reason is worth recording: the boundary suite refused `hostZone()` inside `src/domain` on the very first run, since reading the machine's zone is ambient state and the domain may not read ambient state. `instantOfCivil` is where the correctness risk lives, so its rule is explicit and asserted - candidates come from the offsets a day either side of the naive guess and survive only if they convert back, leaving one candidate for an ordinary time, two for a repeated fall-back hour (the earlier instant wins) and none for a skipped spring-forward hour (the pre-transition offset shifts forward), all three matching the convention platform local-time parsing uses. Twelve cases cover UTC, a half-hour zone, both Berlin transitions, the southern hemisphere, the previous-day western case and a round trip at four instants including 1999-12-31T23:59:59Z; five mutations of the module all bite. **The box stays open**, because the wiring is its actual condition: `localDateKey` and `formatClockTime` still read the ambient zone and the schedule and calendar modules carry about a hundred `Date` getter reads between them. Agenda: 51 open of 1177, unchanged - a foundation is not a tick. |
| Branch | `stage7-record-store-contract` |
| Last audited SHA | `a3bede7` - source/test typecheck 0, build 0, `git diff --check` 0, `npm test` 0 under default parallelism: **253 test files / 1673 passed / 1 skipped**, run 2026-09-12 in this working tree (252 / 1661 at `9670c15`, and the counts below that are unchanged). One suite arrived with this slice, `tests/timeZone.test.ts` (twelve cases), and five mutations of the new module all bite with the source restored byte for byte. Two things this row states rather than implies: the slice adds a *type* and a *boundary reader* rather than changing any derivation, which is why `npm test` grew by one file and not by failures; and the boundary suite rejected the first draft of the module, so the rule that caught it is live rather than decorative. |

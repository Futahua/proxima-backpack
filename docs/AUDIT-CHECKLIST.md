...
| Awaiting the creator | **44 open boxes, and the split is measured rather than estimated.** **37 need the creator at the machine or their data**: sixteen native creator-vault acceptances (Gate 6.1A-6.1P), the clean-profile and real-grant proofs (5.1, 5.2), nine Obsidian-coexistence boxes (Gate 13), the release-readiness revalidations (Gate 22, 22R), the fresh native acceptance for `071f8c3`, Gate 20's native grant-registry half, Gate 14.1's Obsidian round-trip and external-concurrent-edit boxes, and Gate 11.3's real FSA acceptance - which was listed here as automatable until it was examined: a *picked* real Windows directory needs one human gesture, and no automated run can produce it. **3 are a decision only the creator can make**: Gate 14.1's "Previous write-model gate signed off", "Explicit creator decision to allow writes", and "One smallest record type enabled first". The gate itself is built and refuses by default (`src/app/writeAdmission.ts`, D83) - nothing in this repository can enroll it, by design and by test. **4 need a Papers change this work may not make, and the creator has since been asked and declined it**: the three native-source handlers (`grant`/`open`/`reveal`) are in Papers' *production* preload and absent from the developer-control preload the automated runs use, so on a diagnostic instance the page's requests are answered with silence and Gate 9.3's four boxes stay unobservable from here. Measured on 2026-09-13: the production preload carries them at `src/preload/backpackProject.ts` lines 178-217 and `src/preload/backpackProjectDevControl.ts` carries none of them. The question was then put to the creator with the change's scope stated in plain language, and they chose to leave Papers untouched - so this is a decision on record rather than an outstanding permission. **0 are implementable in this repository**: Gate 4.4's browser-native-handle box closed at `e315470`, and what is left is a person, a granted directory, or a host change. |

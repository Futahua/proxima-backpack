...
| Field | Value |
| --- | --- |
| Current slice | **The write model's evidence half is proven, at `e83dcca`.** Gate 14 asks for proof rather than assurance, so `tests/writeAfterImage.test.ts` puts a disposable vault in front of the real conditional writer and the project's own whole-tree fingerprint: a committed update changes exactly the declared file (asserted as the changed-path set *and* as bytes for every other file), two declared writes leave nothing extra, and a refused stale write leaves the tree byte-identical while naming the revision that beat it. The case that makes the evidence load-bearing is the control pair - a same-size edit with its timestamp handed back exactly is invisible to a size-and-mtime sweep of the same tree and caught by the content-hashed one - and four probes turn the suite red, including the one the suite itself found: `utimes` cannot restore sub-millisecond precision, so the clock is normalised to a whole second first. The visible-refusal box moved with it: the producer half is measured, and what remains is that `setRefusal(reason: string)` drops the detail and the revision before the surface sees them. Agenda: **47 open of 1177**, from 48. |
| Branch | `stage7-record-store-contract` |
| Last audited SHA | `e83dcca` - source/test typecheck 0, build 0, `git diff --check` 0, `npm test` 0 under default parallelism: **255 test files / 1691 passed / 1 skipped**, run 2026-09-12 in this working tree. This slice added one test file and no product code, which is the point: the evidence it asserts was previously a claim about the writer rather than a measurement of the tree. The test typecheck found one real defect the build does not compile - `FIXTURE[path]` is `string | undefined` under `noUncheckedIndexedAccess`, fixed by naming the edited record once rather than by asserting it away. |
...
- [ ] Stale write visibly refused. — **one half is now measured at `e83dcca` @ `2026-09-12T21:23:31+07:00`:** the producer side is evidence
      rather than assumption, because a refused stale write is proven to leave the tree byte-identical *and* to name
      the revision that beat the caller (`tests/writeAfterImage.test.ts`), and the record-store surfaces do render a
      refusal already - `elastic-drop-refusal`, `task-editor-refusal`, `schedule-write-refusal`, and the canvas and
      board refusals, each asserted by its own suite. **What stays open is that the rendered refusal is a bare code:**
      every write action's `deps.setRefusal(reason: string)` takes the code and drops `TaskMutationFailure.detail`
      and `.actualRevision` on the way, so a reader is shown `stale-revision` rather than the sentence the producer
      already wrote ("another writer changed this task first", now at revision N). The next slice threads that detail
      through the boundary and asserts the rendered text.

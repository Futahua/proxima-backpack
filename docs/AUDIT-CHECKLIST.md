...
| Last audited SHA | `a7bf33e` - fixture generation 0, source/test typecheck 0, build 0, `git diff --check` 0, vitest 0 under default parallelism: 232 test files / 1528 tests. The file count is checked against the tree rather than quoted: 232 `*.test.ts` files exist and the runner collects all 232, which is a check worth keeping because another repository in this workspace turned out to have three test files no script ran. |

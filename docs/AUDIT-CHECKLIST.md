...
| Field | Value |
| --- | --- |
| Current slice | **Stage 17's contract matrix is complete at `f9c202a`, and the cross-cutting slice it exposed is the live task.** The matrix is 36 rows / 360 cells - 248 satisfied, 111 gaps, one not applicable - one table per family (task, event, project, workflow-stage, schema) in `tests/actionCoverageAudit.test.ts`, each cell satisfied, a gap, or not applicable with its reason, each gap an asserted absence with a witness beside it so it fails the day it closes, and each table locked to exactly the rows the file already carries. What the gaps say, grouped: 36 rows have no untrusted boundary accepting `unknown`; 36 carry no system-generated request id (the coordinator mints one for update and delete, the semantic result discards it, and creates never reach the coordinator); 36 emit no semantic event (the only event ring belongs to the dispatcher, which refuses every record verb with `action-not-available` before the record layer); the retained task recurrence is unwired and unobserved; the schema field edit is never read back; and `project.delete` answers with counts rather than ids. The AUTHOR's ruling on what closes the first three is recorded below and in the parity checklist: a separate semantic request id minted at the semantic-action boundary, a terminal semantic event through an injected audit sink after convergence with a partial outcome of its own, and no fabricated revision. Behind that, at `a983b9a`, the composer's Execute became a listener a test can click - the binding extracted into `src/browser/templateExecuteBinding.ts`, `main.ts` composing it with no branch of its own, `tests/templateExecuteClick.test.ts` clicking the real chain into a real store, and `tests/templateCallerEquivalence.test.ts` comparing that click against a `submitTemplateExecution` submission - which is what closed the two boxes the AUTHOR had reopened. Earlier: Stage 16 complete through the Templates row at `4d0bfaa`; Stage 16 slice 1 at `d0993d9`; Stage 17 record writes and Stage 20 chrome at `425a631`. |
| Branch | `stage7-record-store-contract` |
| Last audited SHA | `f9c202a` - fixture generation 0, source/test typecheck 0, build 0, `git diff --check` 0, vitest 0 under default parallelism: 229 test files / 1506 tests. The file count is checked against the tree rather than quoted: 229 `*.test.ts` files exist and the runner collects all 229, which is a check worth keeping because another repository in this workspace turned out to have three test files no script ran. |
| Parity agenda | `D:\Letters\MatTroiSeConMoc\LongHorizon\proxima\proxima-full-parity-checklist.md` (branch `codex/reviewer-send-verification`) carries the full-parity agenda this work is driven by: **789 ticked / 51 open** at `0f097d6` counting every box the document carries, or **698 / 50** counting only top-level boxes. The pair moved back up after the AUTHOR's reopened boxes were closed on its own check of the evidence (`a983b9a`), and both conventions are stated because the figures on this row have moved before for no reason other than the counting: 763 / 77 and 686 / 62 were earlier revisions under a different total. |

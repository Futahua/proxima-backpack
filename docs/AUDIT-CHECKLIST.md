...
| Field | Value |
| --- | --- |
| Current slice | **The runtime reconciles its recovery journal on the boot rather than on the first write gesture, at `9670c15`.** Gate 13 had carried that gap in two places - 13.C's last box, and 13.C2's closing sentence "runtime wiring remains open" - and both were the same missing call: `resolveBrowserTaskMutations` ran the gate only when a write was attempted and only after the store reported itself activated, so a store nobody wrote to was a store whose journal nobody read, including the boot after a crash. `StartupSessionOptions.runRecovery` is the seam, the call sits inside `start()` before the session is handed to anybody, and the answer is reported as one of four distinct states rather than folded into a verdict: `reconciled`, `blocked`, `failed` (a hook that throws leaves the product usable, because recovery gates mutation authority rather than reading) and `not-run` for a boot that supplied no hook at all. Nothing is granted on the answer. The browser boot supplies the hook through `resolveBrowserRecoveryStartup`, which composes the same backend, journal and gate the write path composes, so there is one composition instead of two that can drift. Five mutations of the wiring were written and all five bite, with the file restored byte for byte: the call removed, a throwing hook swallowed as clean, a blocked answer reported as reconciled, the failed code dropped, and a missing hook claiming a clean boot. One honesty note about this slice's own accounting: 13.C2's box was **already ticked** and what changed there is its text, which no longer says the wiring is open; only 13.C's box flipped, which is why the agenda moved by one rather than two. |
| Branch | `stage7-record-store-contract` |
| Last audited SHA | `9670c15` - source/test typecheck 0, build 0, `git diff --check` 0 on this commit, `npm test` 0 under default parallelism: **252 test files / 1661 passed / 1 skipped**, run 2026-09-12 in this working tree (251 / 1655 at `e08616d`, and the counts below it are unchanged). One suite arrived with this slice, `tests/startupRecoveryWiring.test.ts`, and it is the only new file: the slice is six cases, five mutations and the wiring itself. The mutations are the reason the wiring is provable rather than present - each one is a plausible shorthand (drop the call, treat a throw as success, report a blocked answer as reconciled, stop naming the failure, let a missing hook look clean) and each one fails the suite - and the integration case is what keeps the rest honest: the hook is the real `startRecordMutationAuthority`, over a durable journal seeded before a restart with the intended bytes already on disk, so the `effect-present` classification and the journal's transition to `committed` come back through the wiring rather than from a stub. The agenda stands at **52 open of 1177** (1125 ticked), down from 53; the earlier record's "from 53" in this slice's commit message is a miscount and this row is the correction, made the same minute rather than left to sit. |

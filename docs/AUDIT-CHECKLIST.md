...
| Field | Value |
| --- | --- |
| Current slice | **The semantic envelope is being wired action by action, at `a7bf33e`.** The contract exists (`src/app/semanticAudit.ts`: accepted / rejected / **partial** as its own outcome, one bounded event, the sink, and the single rule that decides the outcome) and five task rows are covered: `task.create` at `e0d097e`, and the editor's Save and Delete at `a7bf33e`, which carry update, delete, dates and weight/duration. Each action mints its id before its first refusal and appends one terminal event after convergence, through the ring-backed sink in `ProximaActionDispatcher.auditSemantic` - which stamps the dispatcher's real state revision, never one the record layer invented. The matrix's cells are per row now, so wiring one action flips that row's two cells while its siblings keep asserting the gap. **Next in order:** `task.execution.move`, the bulk report (its own table, because a selection reports one outcome per member), then the event, project, workflow-stage and schema families; after the wiring, the eight open agent-protocol boxes - including the dispatcher's `action-not-available` refusal for every record verb, which the matrix surfaced and which is HARD GATE E's question in its sharpest form. Behind that: the complete contract matrix at `f9c202a` (36 rows / 360 cells), the clickable Execute binding at `a983b9a`, and Stage 16 complete through the Templates row at `4d0bfaa`. Earlier: Stage 16 slice 1 at `d0993d9`; Stage 17 record writes and Stage 20 chrome at `425a631`. |
| Branch | `stage7-record-store-contract` |
| Last audited SHA | `c81bc55` - fixture generation 0, source/test typecheck 0, build 0, `git diff --check` 0, vitest 0 under default parallelism: 233 test files / 1534 tests. The file count is checked against the tree rather than quoted: 233 `*.test.ts` files exist and the runner collects all 233, which is a check worth keeping because another repository in this workspace turned out to have three test files no script ran. |

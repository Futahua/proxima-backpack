...
| Last audited SHA | `4b241f8` - fixture generation 0, source/test typecheck 0, build 0, `git diff --check` 0, vitest 0 under default parallelism: 235 test files / 1546 tests. The file count is checked against the tree rather than quoted: 235 `*.test.ts` files exist and the runner collects all 235, which is a check worth keeping because another repository in this workspace turned out to have three test files no script ran. |

...
| Field | Value |
| --- | --- |
| Current slice | **The recovery story became visible, at `0c80b85`.** The machinery - durable journal, `not-applied`/`effect-present`/`conflict`/`already-committed` classification, and a *conditional* restore that refuses to overwrite a record someone else has changed - was already built and tested, and reconciliation already ran on every boot before mutation authority was handed out. What it lacked was a reader: the answer went to the acceptance report and stopped there. `src/browser/recoveryNotice.ts` renders it in the header on every boot, including the clean one, and `not-run` never renders as `clean` - a boot that never asked and a boot that asked and found nothing differ in machine value, tone and sentence, because that difference is the whole point after a crash. A reconciliation is clean only when it resolved everything it found *and* left mutation authority available. The byte-level restore's lack of a production caller is stated in the box rather than implied away. Decisions: D84. Agenda: **45 open of 1177**, from 46. |
| Branch | `stage7-record-store-contract` |
| Last audited SHA | `0c80b85` - source/test typecheck 0, build 0, `git diff --check` 0, `npm test` 0 under default parallelism: **257 test files / 1703 passed / 1 skipped**, run 2026-09-12 in this working tree. This slice added one browser module and one suite, and rendered a state that already existed rather than inventing one. Five mutations of the notice bite with every file restored byte for byte, including the one that matters: an unasked recovery rendering as clean. |
...
| Awaiting the creator | **45 open boxes, and the split is measured rather than estimated.** **36 need the creator at the machine or their data**: sixteen native creator-vault acceptances (Gate 6.1A-6.1P), the clean-profile and real-grant proofs (5.1, 5.2), nine Obsidian-coexistence boxes (Gate 13), the release-readiness revalidations (Gate 22, 22R), the fresh native acceptance for `071f8c3`, Gate 20's native grant-registry half, and Gate 14.1's Obsidian round-trip and external-concurrent-edit boxes. **3 are a decision only the creator can make**: Gate 14.1's "Previous write-model gate signed off", "Explicit creator decision to allow writes", and "One smallest record type enabled first". The gate itself is built and refuses by default (`src/app/writeAdmission.ts`, D83) - nothing in this repository can enroll it, by design and by test. **4 need a Papers change this work may not make**: the three native-source handlers (`grant`/`open`/`reveal`) are in Papers' *production* preload and absent from the developer-control preload the automated runs use, so the page's requests are answered with silence and Gate 9.3's four boxes cannot be observed from here. **2 are automatable through the host seam and have not been attempted**: Gate 4.4's "browser-native `FileSystemDirectoryHandle` use tested without user picker" and Gate 11.3's "real FSA acceptance uses disposable actual Windows directories" - both need a Papers instance driving a disposable directory through the automation seam, which is a run rather than a decision. |

...
| Field | Value |
| --- | --- |
| Current slice | **The composer's Execute is a listener a test can click, at `a983b9a`**, which is the slice the browser AUTHOR scoped after rejecting the two-half evidence on Stage 17's *UI invocation test exists*. `src/browser/templateExecuteBinding.ts` holds the binding - one delegated listener that resolves the control from the event, because the panel re-renders under it, and a re-entry guard because a disabled button is presentation while a second click creating the same tasks twice is the rule - and `main.ts` composes it with `bindTemplateExecute(root)` and no branch of its own, so the verb has one handler. `tests/templateExecuteClick.test.ts` renders the real composer, binds that module, clicks `data-c1-key="template-execute"` through the existing `InteractionHarness` and asserts the records in a real store, the panel's ids against the landed ids, the running state, no double run, a refusal with the composer open, and a partial run reported with the id that landed; mutating the listener's verb fails all five. `tests/templateCallerEquivalence.test.ts` compares the click and a `submitTemplateExecution` submission in two isolated stores on what each caller reports and on the normalised records. Before that, Stage 16 completed through the Templates row at `4d0bfaa`: a template is executable from both directions - `parseTemplatePlan` into `executeTemplatePlan` through the injected `TaskCreateOperations` port, with no RecordStore import, no id allocation of the executor's own and no clock it does not use; one canonical submission path (`templateExecuteAction`) that parses the text itself, so no caller can execute a hand-built plan; an agent entry (`templateSubmission`) carrying the `template.execute` wire shape as a **sibling entry rather than a `ProximaAction`**; and the composer panel's Execute. Earlier: Stage 16 slice 1 at `d0993d9`, Stage 17 record writes and Stage 20 chrome at `425a631`. |
| Branch | `stage7-record-store-contract` |
| Last audited SHA | `a983b9a` - fixture generation 0, source/test typecheck 0, build 0, `git diff --check` 0, vitest 0 under default parallelism: 229 test files / 1500 tests. The file count is checked against the tree rather than quoted: 229 `*.test.ts` files exist and the runner collects all 229, which is a check worth keeping because another repository in this workspace turned out to have three test files no script ran. |
| Parity agenda | `D:\Letters\MatTroiSeConMoc\LongHorizon\proxima\proxima-full-parity-checklist.md` (branch `codex/reviewer-send-verification`) carries the full-parity agenda this work is driven by: **787 ticked / 53 open** at `e7c2724` counting every box the document carries, or **696 / 52** counting only top-level boxes - two boxes were reopened there by the AUTHOR's ruling, so the count went down rather than up. Both conventions are stated because the figures on this row have moved before for no reason other than the counting: 763 / 77 and 686 / 62 were earlier revisions under a different total. |

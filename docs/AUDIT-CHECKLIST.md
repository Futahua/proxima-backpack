...
| Field | Value |
| --- | --- |
| Current slice | **The D60 project-delete cascade landed at `21de194`, and it was the last product question in the program.** The creator chose the explicit cascade: a delete submits the task and event ids it confirmed, the operation verifies them against the store before removing anything (members first, the project last), and the answer is `'deleted'` with every affected entity id rather than the counts the old open-question refusal reported. A list that no longer matches is `membership-mismatch` and writes nothing; the revision is still checked before the membership question. The Hub asks before it acts - the first press captures the membership it is drawing and shows the ids, the second submits exactly that list, and an empty project is one press - and the agent wire carries the same list and refuses a delete that names a project without it. Full suite **260 test files / 1727 passed / 1 skipped, exit 0**; typecheck 0; `git diff --check` 0. The parity agenda this work is driven by is now **840 of 840 ticked, 0 open**, the two D60-gated boxes having closed with this landing. |
| Branch | `stage7-record-store-contract` |
| Last audited SHA | `21de194` @ `2026-09-13T00:01:53+07:00` - source/test typecheck 0, build 0, `git diff --check` 0, `npm test` 0 under default parallelism: **260 test files / 1727 passed / 1 skipped, exit 0**, run 2026-09-13 in this working tree. This slice implemented D60 from the AUTHOR's seven packets (1/A/B0/B1/B2/B2-agent/B3), squashed onto the accepted branch from `wip/d60-project-delete-cascade` - kept at `2a541f6` as the packet-by-packet record - and repaired six module headers and doc comments, one contract row and four suites whose text still described the retired open-question refusal. Three executor repairs are declared on the commit and all three are transport: where the pending-delete state lives, a Cancel control the modal needed to have a way out, and the split that keeps the delete arm uncallable without members. |
| Parity agenda | `D:\Letters\MatTroiSeConMoc\LongHorizon\proxima\proxima-full-parity-checklist.md` (branch `codex/reviewer-send-verification`) carries the full-parity agenda this work is driven by: **840 ticked / 0 open** counting every box the document carries, or **748 / 0** counting only top-level boxes. The last four closed in two landings: D86's template-local relations and D62's retained task recurrence at `24e93c8`, and the two D60-gated boxes - `affected entity IDs returned` and the release gate `Every such action has typed success/refusal/stale/failure` - at `21de194`. The tick audit was re-run rather than carried: 840 boxes, no ticked box without a SHA, no false timestamp, **954 SHA references and every one resolving** except the by-design citation of the creator's live Obsidian plugin, which the scan excludes because it is live creator data this work stays out of. |
| Papers changed | **One Papers change is now recorded, on its own branch and with the creator's authorization.** Papers `9a839eb` @ `2026-09-13T07:24:03+07:00` (branch `native-source-diagnostic-preload`, cut from the accepted host branch `gate10-relay` @ `d2a3c74`) mirrors the shipping preload's three native-source requests and its `validNativeSourceRef` check into the developer-control preload, so a diagnostic instance answers a page's `native-source-grant`/`-open`/`-reveal` instead of staying silent. It is what made Gate 9.3's four refusal observations possible, and it changed nothing else: no release, no installation, no restart, no ordinary-behaviour change, and the same three narrowly typed requests with the same ownership scoping, which is the scope the creator authorized after it was stated in plain language. Papers' own suite was run for it - typecheck 0, `npm test` 0 at **99 files / 942 passed / 4 skipped**, build 0. The three Gate 2.8 requests remain open and unimplemented: a refused semantic-keys payload is refused in silence, three `tools/*.mjs` CLIs are no-ops on Windows, and an unpresented surface answers a capture with an artifact-bound error. |
...
| Awaiting the creator | **43 open boxes, and the split is measured rather than estimated.** **37 need the creator at the machine or their data**: sixteen native creator-vault acceptances (Gate 6.1A-6.1P), the clean-profile and real-grant proofs (5.1, 5.2), nine Obsidian-coexistence boxes (Gate 13), the release-readiness revalidations (Gate 22, 22R), the fresh native acceptance for `071f8c3`, Gate 20's native grant-registry half, Gate 14.1's Obsidian round-trip and external-concurrent-edit boxes, and Gate 11.3's real FSA acceptance - which was listed here as automatable until it was examined: a *picked* real Windows directory needs one human gesture, and no automated run can produce it. **3 are a decision only the creator can make**: Gate 14.1's "Previous write-model gate signed off", "Explicit creator decision to allow writes", and "One smallest record type enabled first". The gate itself is built and refuses by default (`src/app/writeAdmission.ts`, D83) - nothing in this repository can enroll it, by design and by test. **3 are the positive half of a gate whose host change is now made**: Papers `9a839eb` mirrors the three native-source requests into the developer-control preload, so the four refusals became observable and Gate 9.3's refusal box closed at `7fc92b9`. What is left of that gate is opening and revealing a *real* source - which needs an OS-backed `File` from a drag-and-drop - and the staleness check that lives behind a granted reference; both are the creator's hands rather than a host change, and neither was ever a Papers question. **0 are implementable in this repository**: Gate 4.4's browser-native-handle box closed at `e315470`, and what is left is a person, a granted directory, or the creator's own hands. |
...
- [x] Browser-native `FileSystemDirectoryHandle` use tested without user picker. — `e315470` @ `2026-09-12T21:42:13+07:00`
      *(`tools/fsa-native-handle-probe.mjs`, `npm run probe:fsa-handle`, evidence at
      `D:\Letters\MatTroiSeConMoc\.dsh\gate4-native-handle\report.json`.)* **What it runs:** the shipped adapter against a
      real `FileSystemDirectoryHandle` obtained from `navigator.storage.getDirectory()`, inside headless Chrome over the
      loopback viewer, in a disposable profile - no picker, no gesture, no grant, and the picker is *wrapped and counted*
      so the run reports `showDirectoryPicker calls: 0` rather than leaving it to be assumed. **What it establishes, nine
      stages, each recorded:** the handle is native (`instanceof FileSystemDirectoryHandle`, constructor name checked, and
      the native-only `isSameEntry`/`resolve` present, which the structural subset cannot have); `queryPermission` exists and
      answers `granted`; a fixture written through the *native* API; the repository contract holds over real browser storage
      (list, walk, exists, a bounded read whose revision is shaped `iso:size:hash8`, and three refusals); and a second page
      load reads the same three files through a freshly obtained handle. **The finding the probes produced is worth more than
      the PASS:** with the traversal guard deleted the '..' read is *still* refused - a name-based handle simply has no
      child called `..` - so a first assertion that accepted any error let that probe survive. The contract assertion now
      requires each refusal by message, and all three probes bite. The recorded conclusion is that for a native handle the
      traversal guard is a **refusal-quality guard and the handle is the security boundary**, which is a different claim
      from the one the guard looks like it is making. **What it does not claim:** a *picked* real Windows directory, whose
      permission and persistence behaviour still belong to Gate 5 and need one human gesture.
...
The unit test still uses the structural subset, because a Node runner has no browser storage to offer it. The
*native* claims are now measured rather than deferred: `tools/fsa-native-handle-probe.mjs` runs the adapter against a real
OPFS handle and records the permission answer and the survival of the bytes across a page load, neither of which the double
can produce. What remains with Gate 5 is the *picked* directory - a real Windows folder chosen by a person - and the same
tool states that limit instead of implying it.
...
- [x] Unauthorized source rejected. — `9a839eb` @ `2026-09-13T07:24:03+07:00` and `7fc92b9` @ `2026-09-13T07:24:10+07:00` *(**observable now, which is what this box was waiting for.** The probe attempts four refusals from a page - a fabricated `File` (the host resolves real paths itself, so a page cannot name one), an ungranted reference, a machine path, and a non-reference string - and on a diagnostic instance all four used to be answered with **silence**, recorded as `no-answer`, because silence is not evidence of authorization. Two commits close it, one per repository. **Papers `9a839eb`** (branch `native-source-diagnostic-preload`, creator-authorized on 2026-09-13) mirrors the shipping preload's three native-source requests and its `validNativeSourceRef` into the developer-control preload, with the same validation and the same host channels; the host side needed no change, because `hostIpc.ts` already routes all three by `event.sender.id`, which is what scopes a grant to its owning Backpack. **This repository `7fc92b9`** fixes the instrument: the probe built its answer map from the six containment questions while the verdict loop iterated the questions the run actually asks, so under `--handoff true` the four handoff answers were always null and a run in which every refusal had really been observed still reported BLOCKED with four empty answers. Both repairs are declared rather than folded in: the preload mirror is a host change, and the probe fix is repair of the instrument that measures it. **The acceptance, run against an instance built from that source and reporting `d2a3c74+local`, `packaged false`:** `npm run probe:containment -- --handoff true` exits **0** with status **PASS** and ten expectations satisfied - the six containment answers unchanged, and `handoff-fabricated`, `handoff-ungranted`, `handoff-machinePath` and `handoff-malformed` all reading `refused`. The report is kept at `D:\Letters\MatTroiSeConMoc\.dsh\containment-handoff\probe-report.json`. The positive half is untouched and still needs a person: a real open or reveal needs an OS-backed `File`, which only a drag-and-drop can produce.)*
...
- [x] Stale write visibly refused. — `79c7acd` @ `2026-09-12T21:29:06+07:00` *(the refusal now reaches the surface
      saying what happened and at which revision, where before it arrived as a bare code. `src/app/refusalPresentation.ts`
      is the one place that writes a refusal down for a reader: the machine code first, deliberately, because selectors
      and agents match on it, then the producer's own bounded sentence, then the revision that beat the caller - and
      nothing invented when the producer supplied nothing, since a formatter that guessed a revision number would be
      worse than the code it replaced. Applied at `performElasticDrop` and at both write sites of
      `saveTaskFromEditor`/`deleteTaskFromEditor`, the places a lost race lands on a person. Each case drives a real
      store, has a second writer move the record on, and asserts what the surface is handed *and* that the loser's
      claim is still a refusal: the winner's write stands and the refused card did not move. **Visibility itself was
      already there and is named rather than re-claimed** - the record-store surfaces render their refusals
      (`elastic-drop-refusal`, `task-editor-refusal`, `schedule-write-refusal`, the canvas and board refusals, each
      asserted by its own suite). What this bought is that a reader is told `stale-revision - another writer changed
      this task first (now at revision X)` instead of `stale-revision`. The full suite caught the one place that had
      pinned the old contract - `tests/taskMoveGesture.test.ts` asserted `[null, 'stale-revision']` at the shell-glue
      boundary, and now pins the whole string including the revision - which is the argument for running all 256 files
      rather than the ones the slice touched. Six mutations bite. **Scoped honestly:** the sinks this slice did not
      touch (`eventWriteActions`, `projectLifecycleActions`, `workflowStageWriteActions`, `timelineChangeAction`,
      `eventRecurrenceActions`) still hand their surface a code or a detail without the revision.)*
- [x] Evidence proves no unrelated bytes changed. — `e83dcca` @ `2026-09-12T21:23:31+07:00` *(the proof is a
      disposable vault, the conditional disk writer's own compare-and-swap commit, and the project's whole-tree
      fingerprint taken before and after - the same implementation the zero-write acceptance uses, so there is one
      definition of "what changed" rather than two that can disagree. Four cases: a committed update's delta is
      exactly the file it declared, asserted both as the changed-path set and as bytes for every other file in the
      tree; two declared writes leave nothing beyond the declared set; a refused stale write leaves the tree
      byte-identical; and a same-size edit whose timestamp is handed back exactly is invisible to a size-and-mtime
      sweep of the same tree while the content-hashed one catches it. That last case is a control pair, and it is
      what makes the evidence load-bearing rather than decorative. Four probes turn it red - `CONTENT_HASHED_ROOTS`
      emptied, the content hash made constant, the writer touching a file nobody declared, and the stale refusal
      naming the expected rather than the actual revision - each with every file restored byte for byte. One
      measured detail is recorded in the suite rather than assumed: `utimes` cannot restore sub-millisecond
      precision, so the case normalises the clock to a whole second before editing.)*
- [x] Rollback/recovery story exists. — `0c80b85` @ `2026-09-12T21:32:24+07:00` *(the story is assembled from
      parts that were each already tested, and the commit that closes this box is the one that made it
      *visible*. The machinery: a durable journal (`src/app/vaultRecovery.ts`), a classifier that separates
      `not-applied` from `effect-present` from `conflict` from `already-committed`
      (`src/app/vaultRecoveryReconcile.ts`), and a restore that is conditional rather than a rollback
      (`src/app/vaultRecoveryRestore.ts`, which refuses to overwrite a record someone else has since changed),
      each covered by its own suite (`vaultRecoveryCrash`, `vaultRecoveryStartup`, `vaultRecoveryRestore`,
      `recordRecoveryStartup`). The wiring: reconciliation runs on every boot **before mutation authority is
      handed out**, deliberately automatic, because a journal that is only examined on a write is a journal
      nobody reads on the boot that needed it. What was missing was a reader: the answer reached the acceptance
      report through `StartupInspection.recovery` and stopped there. `src/browser/recoveryNotice.ts` renders it,
      and the honest default is the point of the notice - `not-run` is never `clean`, because a boot that never
      asked and a boot that asked and found nothing must not look alike after a crash. A reconciliation is clean
      only when it resolved everything it found *and* left mutation authority available. **Stated rather than
      implied:** the byte-level conditional restore has no production caller yet, so what a live creator-vault
      crash would do remains part of the native acceptance in Gate 6 and Gate 13. Decisions: D84.
      Five mutations of the notice bite.)*

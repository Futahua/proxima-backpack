...
| Field | Value |
| --- | --- |
| Current slice | **A refusal now explains itself, at `79c7acd`.** Every write action reports through `setRefusal(reason: string)` and every caller passed the bare code, while the producers already wrote a bounded sentence and, on a lost race, the revision that beat the caller. `src/app/refusalPresentation.ts` composes them in that order - code first because selectors and agents match on it, then the sentence, then the revision - and invents nothing when a producer supplied nothing. Applied at `performElasticDrop` and both task-editor write sites; each case drives a real store with a second writer and asserts the surface is handed the whole story while the winner's write still stands and the refused card did not move. The full suite caught the one test that had pinned the old contract at the shell-glue boundary. Six mutations bite. Sinks not in this slice are named in the box rather than implied to be covered. Agenda: **46 open of 1177**, from 47. |
| Branch | `stage7-record-store-contract` |
| Last audited SHA | `79c7acd` - source/test typecheck 0, build 0, `git diff --check` 0, `npm test` 0 under default parallelism: **256 test files / 1698 passed / 1 skipped**, run 2026-09-12 in this working tree. This slice added one pure presentation module and touched two write paths; the suite it broke was the evidence that the ritual is not redundant, because a targeted run of the four changed suites passed while the whole run found a case that had pinned the old contract. Six mutations of the composition bite with every file restored byte for byte. |

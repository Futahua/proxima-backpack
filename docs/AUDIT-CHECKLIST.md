...
| Parity agenda | `D:\Letters\MatTroiSeConMoc\LongHorizon\proxima\proxima-full-parity-checklist.md` (branch `codex/reviewer-send-verification`) carries the full-parity agenda this work is driven by: **698 ticked / 50 open** at `6f2bf50`, counted as top-level boxes (the earlier figures here, 763 / 77 and 686 / 62, counted a different box total than the document now has; any-indent counting gives 789 / 51 for the same revision, so the convention matters). |

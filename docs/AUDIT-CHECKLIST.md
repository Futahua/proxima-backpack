...
| Field | Value |
| --- | --- |
| Current slice | **The agent wire runs three families, at `5781e96`.** `task.timeline.change` joined the drop pair and the schema family, and it joined them by the rule rather than by exception: `src/app/timelineChangeAction.ts` now holds `changeTaskSpan`, an operation whose dependency set is the write, the refresh, the id source and the event sink - no projection, no refusal sink, no render - so the record facts it used to read are parameters (`expectedRevision`, and the span as the caller read it). `changeTaskDatesAction` remains the cockpit's entry over that operation. The submission carries the whole span for every gesture, which removes a malformation rather than validating one: a resize that named one end and took the other from the projection is not a shape a caller without a projection can express. Behind it: the schema family on the wire at `2f90e2c`, scoped ordering across surfaces at `b980c33`, the schema action layer at `65f88ef`, the agent write path at `32132bb`, and the contract matrix at `f9c202a` (313 satisfied / 46 gaps / one n/a of 360 cells, counted at the current tree). |

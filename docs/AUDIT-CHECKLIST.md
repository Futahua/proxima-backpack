...
| Field | Value |
| --- | --- |
| Current slice | Stage 16 **complete through the Templates row** at `4d0bfaa`, plus the first Stage 17 cells that could be answered mechanically. A template is now executable from both directions: `parseTemplatePlan` into `executeTemplatePlan` through the injected `TaskCreateOperations` port, with no RecordStore import, no id allocation of the executor's own and no clock it does not use; one canonical submission path (`templateExecuteAction`) that parses the text itself, so no caller - panel or agent - can execute a plan it built by hand; an agent entry (`templateSubmission`) carrying the `template.execute` wire shape as a **sibling entry rather than a `ProximaAction`**, which is the registration the browser AUTHOR asked for; the composer panel's Execute and the shell click chain that reaches it. Stage 17's Templates row is checkable rather than asserted: `tests/actionCoverageAudit.test.ts` names the module and its export, the shell call, a test marker per half, and checks the equivalence column instead of trusting it - and the manual-versus-template comparison was moved up from the executor to the shipped action at `4d0bfaa`, so it compares the path that runs rather than one both callers bypass (D69 judges it on records). **Residual, stated rather than hidden:** the two matrix boxes that were ticked on two-half evidence were **reopened by the browser AUTHOR on 2026-09-12**, because reading `main.ts` as text cannot prove the listener invokes the action under runtime composition. The scoped route is to extract the template execution binding into an importable `src/browser/templateExecuteBinding.ts`, click the stable key the renderer already supplies through the existing `InteractionHarness`, and close the equivalence box by comparing that UI caller against `submitTemplateExecution`. Only *agent invocation test exists* stayed closed; D70 and D71 record the ruling and what would reverse it. Earlier: Stage 16 slice 1 at `d0993d9`, Stage 17 record writes and Stage 20 chrome at `425a631`. |
| Branch | `stage7-record-store-contract` |
| Last audited SHA | `4d0bfaa` - fixture generation 0, source/test typecheck 0, build 0, `git diff --check` 0, vitest 0 under default parallelism: 227 test files / 1492 tests. The file count is checked against the tree rather than quoted: 227 `*.test.ts` files exist and the runner collects all 227, which is a check worth keeping because another repository in this workspace turned out to have three test files no script ran. |

...
| Field | Value |
| --- | --- |
| Current slice | **The host's containment of a Backpack page is measured rather than cited, at `ad6ee56`.** Gate 20's "still sandboxed" box was open on the honest ground that Papers' `papers-backpack://` policy is not this repository's source and had no exact configuration or acceptance evidence here; reading the host's CSP would still only be a reading. A probe Backpack (`tools/containment-probe/`) now answers six questions about its own containment and publishes each answer as a **semantic key**, so the host's own `inspect.visual.elements` carries the result, and `npm run probe:containment` asserts it. On a live Papers 1.3.11 host at `d2a3c74` (`packaged false`) all six came back as containment: no Node runtime visible; a **same-origin** fetch refused, which only a policy can do; indirect eval refused; an embedded frame's document never fetched; the origin enumerating no database but its own; and the page running on the `papers-backpack` origin of its own Backpack id. Three of those answers took more than one attempt, and the attempts are why they can be trusted: an inline module script was refused before it ran (so the probe's own code is an external file), and the frame question answered "loaded" and then "opaque" before resource timing settled it - a refused frame's document is never fetched, and this document's own load is a navigation entry, not a resource one. Gate 20's grant-ownership box keeps its native half open and now records the browser half as measured. Five probes of the checker bite, including a mutation that expects `NODE-VISIBLE` and fails the run. Agenda: **51 open of 1177**, from 52. |
| Branch | `stage7-record-store-contract` |
| Last audited SHA | `a180ab9` (`ad6ee56` carries the tool) - source/test typecheck 0, build 0, `git diff --check` 0, `npm test` 0 under default parallelism: **252 test files / 1661 passed / 1 skipped**, run 2026-09-12 in this working tree. This slice adds no suite and that is stated rather than glossed: its verification is a measurement against a running host, five probes of its own checker, and the two mutations recorded on the containment box - `npm test` is unchanged because the product did not change. The probe project is three files under `tools/containment-probe/` and is registered only into a disposable profile, so nothing here ships. |

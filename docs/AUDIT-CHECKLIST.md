...
| Field | Value |
| --- | --- |
| Current slice | **The C1 surface is visible to the host for the first time, and both reasons it was not are fixed at `e08616d`.** Gate 2's nine evidence boxes were open with one honest reason: no host-integrated C1 artifact existed. The live run found the surface publishing nothing, and the cause was in this project twice over. First the attribute: Papers reads exactly `data-papers-visual-key`, this project's own `AGENTS.md` documents that name, and the renderer emitted `data-c1-key` in 519 places across 64 files, so a surface that had reached `layout-stable` with clean lifecycle diagnostics was invisible to `inspect.visual.elements`. Second the values: Papers validates a key against `^[A-Za-z0-9][A-Za-z0-9._~-]*$` with a 256-key and 128-character bound and refuses the payload **whole and in silence** when one key fails, and the bundled fixture vault holds a task whose declared identity is `Untitled loose task`, so three keys carried a space. Both are fixed here - the rename, and `src/browser/semanticKeyValue.ts` encoding identities into the host alphabet at the single boundary that writes the document - and both are pinned by new suites, including `tests/shippedBundleSemanticKeys.test.ts`, which loads the **built** bundle into happy-dom and asserts the artifact a host actually serves. With both fixed, a fresh window in a live Papers 1.3.11 host answered `layout-stable`, 21 clean lifecycle records, 21 timeline entries, **71 elements / 71 distinct keys / 71 with geometry**, six real assertions, a verified 143 456-byte surface capture and three verified element captures. The same run recorded three host-side findings as requests rather than as work: a refused semantic-keys payload is silent, three `tools/*.mjs` CLIs are no-ops on Windows so the documented `papers:visual-debug` pipeline exits 0 having done nothing, and capturing an unpresented surface answers with an artifact-bound error. Gate 22's C1 integrated-scenario box closes on this acceptance; its two native-acceptance siblings stay open on the creator. |

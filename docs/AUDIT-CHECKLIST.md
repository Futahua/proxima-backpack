...
| Field | Value |
| --- | --- |
| Current slice | **The agent wire runs two families, at `2f90e2c`.** `src/app/agentWritePath.ts` still parses a drop itself - the wire is what knows `from`, `to` and the revision - and now recognises the five `property.schema` verbs and hands them to `src/app/propertySchemaActions.ts`, which owns that family's shape; `submitPropertySchemaAction` accepts a `requestId` from a trusted layer that already minted one, the pattern `moveTaskByGesture` has carried since the drop wrapper, so one submission is one id and one event. The schema operations resolve through a separate optional dependency because the shell has no schema editor yet. Behind it: scoped ordering across surfaces at `b980c33`, the schema action layer at `65f88ef`, the agent write path at `32132bb`, and the contract matrix at `f9c202a` (307 satisfied / 52 gaps / one n/a of 360 cells, counted at the current tree). |
| Branch | `stage7-record-store-contract` |
| Last audited SHA | `2f90e2c` - fixture generation 0, source/test typecheck 0, build 0, `git diff --check` 0, vitest 0 under default parallelism: 241 test files / 1582 tests. The file count is checked against the tree rather than quoted: 241 `*.test.ts` files exist and the runner collects all 241, which is a check worth keeping because another repository in this workspace turned out to have three test files no script ran. Thirteen mutations across four suites were tried this session and twelve failed their suite; the one that did not was aimed at a function's input iteration rather than the sort it performs itself, and retargeting it was caught. The typecheck caught two test-only slips the suite could not see - the fourth time this session that running both mattered - and the contract matrix caught a moved anchor on its own when the schema minting line widened. |

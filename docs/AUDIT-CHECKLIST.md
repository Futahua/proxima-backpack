...
| Field | Value |
| --- | --- |
| Current slice | **A browser-native directory handle is accepted without a picker, at `e315470`.** `tools/fsa-native-handle-probe.mjs` (`npm run probe:fsa-handle`) runs the shipped adapter against a real `FileSystemDirectoryHandle` from `navigator.storage.getDirectory()` in headless Chrome over the loopback viewer, in a disposable profile. Nine stages, each recorded: the handle is native and carries the native-only surface, `queryPermission` answers `granted`, a fixture is written through the native API, the repository contract holds with three message-specific refusals, `showDirectoryPicker` is called **zero** times (wrapped and counted), and a second page load sees the same files through a freshly obtained handle. Three probes bite. The one that first *survived* taught the useful thing: for a name-based handle the traversal guard is a refusal-quality guard, not the security boundary - the handle is - and the acceptance now requires each refusal by message. The picked-directory claims still belong to Gate 5 and still need one human gesture. Agenda: **44 open of 1177**, from 45. |
| Branch | `stage7-record-store-contract` |
| Last audited SHA | `e315470` - source/test typecheck 0, build 0, `git diff --check` 0, `npm test` 0 under default parallelism: **257 test files / 1703 passed / 1 skipped**, plus `npm run probe:fsa-handle` 0 with 9/9 stages, run 2026-09-12 in this working tree. This slice added a tool rather than product code, and the tool is the evidence: a browser acceptance that can fail, with three probes proving it, and a cleanup path that leaves no disposable profile or browser process behind. |
| Parity agenda | `D:\Letters\MatTroiSeConMoc\LongHorizon\proxima\proxima-full-parity-checklist.md` (branch `codex/reviewer-send-verification`) carries the full-parity agenda this work is driven by: **836 ticked / 4 open** counting every box the document carries, or **744 / 4** counting only top-level boxes, after the runtime-validation umbrella closed at `ecf435a`. **All ten "for every row above" umbrellas are now closed** - the two before this one at `a00a749` (`request ID exists` and `event/audit record exists`, each 35 satisfied / one not-applicable / no gaps of 36, re-measured with the in-walker probe rather than adjusted by hand), and this one on D85 at **3 satisfied / 33 not-applicable by decision / 0 gaps**, where the column is answered at the entry that faces an untyped caller rather than by every member module re-parsing a typed request. The four boxes that remain are the creator's product answers (`project.delete` and what happens to a project's members; the template relation syntax for records created together), the operation-only recurrence scope statement, and the typed-outcome cell. The agenda's tick audit was re-run rather than carried: 840 boxes, no ticked box without a SHA, no false timestamp, **947 SHA references and every one resolving** - the one previously unresolvable citation was replaced by the case it meant. |

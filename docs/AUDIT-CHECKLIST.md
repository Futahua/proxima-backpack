...
| Field | Value |
| --- | --- |
| Current slice | **The queue is attached, the agent can write a record, and the queue row is now empty, at `32132bb`.** `src/app/agentWritePath.ts` is the first agent-facing record write entry: a sibling entry (not a `ProximaAction`) whose wire names `task.execution.move`/`task.execution.reorder` and the facts the agent read - including a **required** `expectedRevision` - mints its request id before it parses anything, and hands the work to `moveTaskByGesture`, the same operation the Elastic drop wrapper runs. `performElasticDrop` correspondingly stopped narrowing the gesture's result, which is what makes the two entries comparable as whole objects rather than as a chosen subset of fields, and the dispatcher's containment rule is untouched: the same suite asserts `dispatch` still refuses the same verb with the store byte-identical, beside the write that succeeds. Behind it: the semantic envelope on twenty-four rows at `b4b17b1`, the complete contract matrix (36 rows / 360 cells) at `f9c202a`, the clickable Execute binding at `a983b9a`, and Stage 16 complete through the Templates row at `4d0bfaa`. |
| Branch | `stage7-record-store-contract` |
| Last audited SHA | `32132bb` - fixture generation 0, source/test typecheck 0, build 0, `git diff --check` 0, vitest 0 under default parallelism: 239 test files / 1572 tests. The file count is checked against the tree rather than quoted: 239 `*.test.ts` files exist and the runner collects all 239, which is a check worth keeping because another repository in this workspace turned out to have three test files no script ran. The new tests were mutation-checked rather than trusted: four mutations of the contract (no refresh handed down, the drop wrapper narrowing again, refusals naming no entity ids, and the declared-verb contradiction check removed) each failed the suite. |
| Parity agenda | `D:\Letters\MatTroiSeConMoc\LongHorizon\proxima\proxima-full-parity-checklist.md` (branch `codex/reviewer-send-verification`) carries the full-parity agenda this work is driven by: **814 ticked / 26 open** at `318516e` counting every box the document carries, or **723 / 25** counting only top-level boxes, after the agent write path closed seven boxes (per-record revisions, Stage 0's UI-versus-agent equivalence and its Evidence counterpart, Stage 19's convergence, cockpit-inspection, Board-and-Backlog and race boxes, and the UI+agent concurrent-operation evidence). Both counting conventions are stated because the figures on this row have moved before for no reason other than the counting. |

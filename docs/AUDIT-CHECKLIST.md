...
| Field | Value |
| --- | --- |
| Current slice | **The semantic envelope is being wired action by action, at `a7bf33e`.** The contract exists (`src/app/semanticAudit.ts`: accepted / rejected / **partial** as its own outcome, one bounded event, the sink, and the single rule that decides the outcome) and five task rows are covered: `task.create` at `e0d097e`, and the editor's Save and Delete at `a7bf33e`, which carry update, delete, dates and weight/duration. Each action mints its id before its first refusal and appends one terminal event after convergence, through the ring-backed sink in `ProximaActionDispatcher.auditSemantic` - which stamps the dispatcher's real state revision, never one the record layer invented. The matrix's cells are per row now, so wiring one action flips that row's two cells while its siblings keep asserting the gap. **Next in order:** the eight open agent-protocol boxes - including the finding the matrix surfaced, that the dispatcher refuses every record verb with `action-not-available` before the record layer, which is HARD GATE E's question in its sharpest form. The three schema rows are a different kind of gap and are named as one: there is no semantic action layer on that path, so their envelope cells need a slice of their own rather than two injected fields. Two cells stay open on purpose: the project delete's affected ids, which it reports as counts, and the operation-only rows' absent callers. Behind that: the complete contract matrix at `f9c202a` (36 rows / 360 cells), the clickable Execute binding at `a983b9a`, and Stage 16 complete through the Templates row at `4d0bfaa`. Earlier: Stage 16 slice 1 at `d0993d9`; Stage 17 record writes and Stage 20 chrome at `425a631`. |
| Branch | `stage7-record-store-contract` |
| Last audited SHA | `b4b17b1` - fixture generation 0, source/test typecheck 0, build 0, `git diff --check` 0, vitest 0 under default parallelism: 238 test files / 1565 tests. The file count is checked against the tree rather than quoted: 238 `*.test.ts` files exist and the runner collects all 238, which is a check worth keeping because another repository in this workspace turned out to have three test files no script ran. |
| Parity agenda | `D:\Letters\MatTroiSeConMoc\LongHorizon\proxima\proxima-full-parity-checklist.md` (branch `codex/reviewer-send-verification`) carries the full-parity agenda this work is driven by: **807 ticked / 33 open** at `482631c` counting every box the document carries, or **716 / 32** counting only top-level boxes. The last pass ticked eighteen of the closing evidence statements, each against a test file that was run to confirm it passes - the Elastic cockpit, Timekeeping, the six Schedule modes, projects as workspaces, notes as artifacts, the one status field, the Backlog, recurrence semantics, the read-only agent bridge, legacy bytes preserved, the cutover rule, filenames, ids, relations, schema reprojection, both kinds in one project, crash classification and no silent last-write-wins - which is why the count moves by eighteen rather than by a slice. Both counting conventions are stated because the figures on this row have moved before for no reason other than the counting. |

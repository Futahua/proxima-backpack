...
| Field | Value |
| --- | --- |
| Current slice | Stage 16 slice 1, the template execution core and preflight at `TEMPLATE_SHA`: `executeTemplatePlan` translates a parsed `TemplatePlan` into the same `CreateTaskRequest` the product's task creation already uses, through an injected `TaskCreateOperations` port (the seam the UI calls) and an injected clock - no RecordStore import or receipt, no id allocation of the executor's own because a created id comes back from the port, refusal before the first call for an invalid plan and for the three draft fields whose canonical meaning is undecided (`status`, `completed`, `property.<key>`), and a refusal after a creation reported as partial rather than complete. The browser AUTHOR scoped the slice; its boxes live in the parity checklist below, because this agenda has no Stage 16 section yet - whether it gains one is a structural decision, not one to improvise. Earlier: Stage 17 record writes and Stage 20 chrome at `425a631`. |
...
| Parity agenda | `D:\Letters\MatTroiSeConMoc\LongHorizon\proxima\proxima-full-parity-checklist.md` (branch `codex/reviewer-send-verification`) carries the full-parity agenda this work is driven by: 686 ticked / 62 open at `97c8906` (the earlier figures here, 763 / 77, counted a larger box total than the document now has). |
